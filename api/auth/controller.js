...
exports.register = function(req, res) {
    ...
    DB.query("SELECT * FROM user WHERE email = ?", [user.email], (err, regUser) => {
        if (err) {
        ...
        DB.query(insertQuery, [user.email, user.password, user.salt, 
            config.statuses.user.active, 
            timeFormat, timeFormat], (err, rows) => {
            if (err) {
                return utils.errorResponse(res, 500, 'sysERR_CANT_CREATE_USER');
            }
